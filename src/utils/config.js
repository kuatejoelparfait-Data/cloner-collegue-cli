 ...
 * Synchrone : ne verifie que env + plaintext. Pour verifier aussi le keychain,
 * utiliser `checkApiKeyAsync()`.
...
/**
 * Verifie la presence d'une cle API (env + keychain + plaintext)
 * Version async — a utiliser en priorite quand keytar est disponible.
 * @returns {Promise<{ok: boolean, provider: string, needsKey: boolean, source: string}>}
 */
export async function checkApiKeyAsync() {
  const config = getConfig();
  const provider = config.defaultProvider || 'claude';

  if (provider === 'ollama') {
    return { ok: true, provider, needsKey: false, source: 'none' };
  }
  ...
  const envKey = process.env[envVar] || '';
  if (envKey.trim()) {
    return { ok: true, provider, needsKey: true, source: 'env' };
  }

  // Essayer le keychain
  try {
    const { getApiKey } = await import('./keychain.js');
    const keychainKey = await getApiKey(provider);
    if (keychainKey && keychainKey.trim()) {
      return { ok: true, provider, needsKey: true, source: 'keychain' };
    }
  } catch {
    // keytar indisponible, on continue avec plaintext
  }

  const plainKey = config.providers?.[provider]?.apiKey || '';
  if (plainKey.trim()) {
    return { ok: true, provider, needsKey: true, source: 'plaintext' };
  }

  return { ok: false, provider, needsKey: true, source: 'none' };
}

/**
 * Resout la cle API effective pour un provider (env > keychain > plaintext)
...
export async function resolveApiKey(provider) {
  ...
  try {
    const { getApiKey } = await import('./keychain.js');
    const keychainKey = await getApiKey(provider);
    if (keychainKey && keychainKey.trim()) return keychainKey.trim();
  } catch {
    // keytar indisponible
  }

  const config = getConfig();
  const plainKey = (config.providers?.[provider]?.apiKey || '').trim();
